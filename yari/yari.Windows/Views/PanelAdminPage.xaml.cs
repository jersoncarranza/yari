...
namespace yari.Views
{
    ...
    /// </summary>
    public sealed partial class PanelAdminPage : Page
    {
        ...
        private void cvButton_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (canvas.Name == "cvProgresoEstudiante")
                ProjectionGenerate(-40, cvProgresoEstudiante);
            else if (canvas.Name == "cvRegistroEstudiante")
                ProjectionGenerate(-40, cvRegistroEstudiante);
        }
        ...
        private void cvButton_PointerExited(object sender, PointerRoutedEventArgs e)
        {
            var canvas = (Canvas)sender;
            if (canvas.Name == "cvProgresoEstudiante")
                ProjectionGenerate(0, cvProgresoEstudiante);
            else if (canvas.Name == "cvRegistroEstudiante")
                ProjectionGenerate(0, cvRegistroEstudiante);
        }
        ...
        private void cvButton_Tapped(object sender, TappedRoutedEventArgs e)
        {
            var canvas = (Canvas)sender;
            if (canvas.Name == "cvProgresoEstudiante")
                this.Frame.Navigate(typeof(ProgresoPage),this.oUser);
            else if (canvas.Name == "cvRegistroEstudiante")
                this.Frame.Navigate(typeof(RegistroPage),this.oUser); 
        }
